...
5.SELECT producto.nombre, SUM(ROUD(factura_detalle.cantidad)) AS suma FROM producto INNER JOIN factura_detalle ON producto.id = factura_detalle.producto_id GROUP BY(producto.id) ORDER BY suma DESC;
6.SELECT producto.nombre, SUM(ROUND(factura_detalle.cantidad)) AS suma FROM producto INNER JOIN factura_detalle ON producto.id = factura_detalle.producto_id GROUP BY(producto.id) ORDER BY suma ASC;     
